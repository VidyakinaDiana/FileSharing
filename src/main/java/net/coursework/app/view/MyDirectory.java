...
public class MyDirectory extends HttpServlet {	
    ...
    public void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		pw.println("<div id=\"head\">" +
					"<a href=\"GeneralDirectoryPage\">All files</a>" +
		...
		for (int i = 0; i < packages.size(); i++) {
			if (packages.get(i).getId() == Integer.parseInt(idSession)) {
				File file = new File("FileSharingPro-1.0-Repository//" + packages.get(i).getPath());
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (namePackage != null) {
			if (namePackage.equals("")) {
				resp.sendRedirect("\\FileSharingPro-1.0\\MyDirectoryPage");
				return;
			}
			ArrayList<Package> packages = Main.getDataBase().getPackages();
			for (int i = 0; i < packages.size(); i++) {
			...
			File userPackage = new File("FileSharingPro-1.0-Repository//" + fullNamePackage);
			userPackage.mkdir();
		...
		for (int i = 0; i < packages.size(); i++) {
			if (packages.get(i).getId() == Integer.parseInt(idSession)) {
				File filesInFolder = new File("FileSharingPro-1.0-Repository//" + packages.get(i).getPath());
				...
				for (int j = 0; j < filenames.size(); j++) {
					if (req.getParameter("#" + packages.get(i).getPath() + "//" + filenames.get(j)) != null) {
						File file = new File("FileSharingPro-1.0-Repository//" + packages.get(i).getPath() + "//" + filenames.get(j));
				...
				if (req.getParameter("#" + packages.get(i).getPath()) != null) {
					File file = new File("FileSharingPro-1.0-Repository//" + packages.get(i).getPath());
					recursiveDelete(file);
					File zipFile = new File("FileSharingPro-1.0-Zip//" + packages.get(i).getPath());
				...
				if (req.getPart(packages.get(i).getPath()).getSize() > 0) {
					...
					String finalLocation = System.getProperty("user.dir") + "//FileSharingPro-1.0-Repository//" + packages.get(i).getPath() + "//" + fileName;
